...
char tmp_list_mail[1024] = {0};
static int list_mail(void *data, int argc, char **argv, char **azColName) {
  int in_i;
  fprintf(stderr, "%s: \n", (const char *)data);
  // data = argv[2];
  // printf("%s\n", data);
  // strcat(tmp_list_mail, "\t");
  for (in_i = 0; in_i < argc; in_i++) {
    // printf("%d %d\n", in_i, argc);
    printf("%s = %s\n", azColName[in_i], argv[in_i] ? argv[in_i] : "NULL");
    strcat(tmp_list_mail, argv[in_i] ? argv[in_i] : "NULL");
    if (argc - in_i == 1)
      strcat(tmp_list_mail, "#n#");
    else
      strcat(tmp_list_mail, "#$#");
  }
  printf("\n");
...
int list_index = 0;
int ind = 0;
char re_mail_id[64] = {0};
static int find_mail(void *data, int argc, char **argv, char **azColName) {
  int in_i;
  list_index++;
  if (list_index == ind) {
    fprintf(stderr, "%s: \n", (const char *)data);
    for (in_i = 0; in_i < argc; in_i++) {
      printf("%s = %s\n", azColName[in_i], argv[in_i] ? argv[in_i] : "NULL");
      strcat(re_mail_id, argv[in_i] ? argv[in_i] : "NULL");
      if (argc - in_i != 1)
        strcat(re_mail_id, "#n#");
      // else {
      //   strcat(re_mail_id, argv[in_i]);
      // }
      // printf("****re_mail_id is -[%s]-\n", re_mail_id);
    }
  }
  printf("\n");
...
bool if_find_user = false;
static int find_user(void *data, int argc, char **argv, char **azColName) {
  int in_i;
  fprintf(stderr, "%s: \n", (const char *)data);
  // data = argv[2];
  // printf("%s\n", data);
...
char return_post_id[1024] = {0};
static int find_postid(void *data, int argc, char **argv, char **azColName) {
  int in_i;
  char tmp[1024] = {0};
  fprintf(stderr, "%s: \n", (const char *)data);
  // for (in_i = 0; in_i < argc; in_i++) {
  ...
  strcat(return_post_id, tmp);
  printf("\n");
  return 0;
}

char return_mail_id[1024] = {0};
static int find_mailid(void *data, int argc, char **argv, char **azColName) {
  int in_i;
  char tmp[1024] = {0};
  fprintf(stderr, "%s: \n", (const char *)data);
  sprintf(tmp, "%s", argv[0]);
  strcat(return_mail_id, tmp);
  printf("\n");
  return 0;
}
...
void find_user_fd(int index) {
  ...
  for (vec_i = 0; vec_i < sizeof(vec); vec_i++) {
    if (vec[vec_i].user_fd == index) {
      ...
      // printf("find--> %d (%s) %d\n", vec[vec_i].user_fd,
      // vec[vec_i].user_name, vec[vec_i].nw_state);
    }
  }
}
...
int main(int argc, char const *argv[]) {
  ...
  // sql part
  sqlite3 *client_db, *board_db, *post_db, *comment_db, *mail_db;
  char *zErrMsg = 0;
  int rc;
  ...
  const char *data = "Callback function called",
             ...
             *insert_to_mail =
                 "INSERT INTO MAIL (Subject, FromWho, Date, ToWho)",
             *create_table =
                 "CREATE TABLE CLIENT("
  ...
  rc = sqlite3_open("mail.db", &mail_db);
  if (rc) {
    fprintf(stderr, "Can't open database: %s\n", sqlite3_errmsg(mail_db));
    exit(1);
  } else {
    fprintf(stderr, "Opened database successfully\n");
  }
  ...
  rc = sqlite3_exec(mail_db, create_mail, callback, (void *)data, &zErrMsg);
  if (rc != SQLITE_OK) {
    fprintf(stderr, "SQL error: %s\n", zErrMsg);
    sqlite3_free(zErrMsg);
  } else {
    fprintf(stdout, "Create mail successfully\n");
  }
  ...
  while (1) {
    ...
    for (int index = 5; index <= fdmax; index++) {
      if (FD_ISSET(index, &read_fds) && index != listener) {
            ...
            if ((foundboard == std::string::npos) ||
              ...
              if (if_find_board == false) {
                ...
                if (rc != SQLITE_OK) {
                  ...
                  char ttmpp[1024] = {0};
                  sprintf(ttmpp,
                          "select PostID from POST where Title = \'%s\' AND "
                          "Content = \'%s\' "
                          "AND Author = \'%s\';",
                          title, content, now_user);
                  rc = sqlite3_exec(post_db, ttmpp, find_postid, (void *)data,
                                    &zErrMsg);
                  if (rc != SQLITE_OK) {
                    fprintf(stderr, "SQL error: %s\n", zErrMsg);
                    sqlite3_free(zErrMsg);
                    ...
                    fprintf(stdout, "find post id successfully\n");
                    // sprintf(tmp, "Create board successfully.\n");
                    printf("the id is (%s)\n", return_post_id);
                    strcat(tmp, return_post_id);
                    strcat(tmp, "#");
                    strcat(tmp, now_user);
                    strcat(tmp, "#");
                    memset(return_post_id, 0, sizeof(return_post_id));
                  }
              ...
              //   sleep(1);
              //   send(index, str_content.c_str(), str_content.size(), 0);
              // }
              send_content = false;
            }
          }
        ...
        // --------------------------list-post--------------------------
          ...
          if (str_cmd.length() == 9) {
            ...
            if (if_find_board == false) {
              ...
              memset(tmp_list_splcmd, 0, sizeof(tmp_list_splcmd));
              sprintf(list_post_msg, "\tID\tTitle\tAuthor\tDate\n");
              find_user_fd(index);
            ...
            if ((found_title == std::string::npos) &&
              ...
              if (strlen(return_postowner) == 0) {
                ...
                if (strcmp(now_user, return_postowner) != 0) {
                  ...
                  memset(tmp, 0, sizeof(tmp));
                  sprintf(tmp, "%s#Update successfully.\n%% ", now_user);
          ...
          if (now_state == VISITOR) {
            ...
            if (strlen(return_postowner) == 0) {
              ...
              if (strcmp(now_user, return_postowner) != 0) {
                ...
                memset(tmp, 0, sizeof(tmp));
                sprintf(tmp, "%s#Delete successfully.\n%% ", now_user);
                // std::string response = "Update successfully.\n% ";
                send(index, tmp, sizeof(tmp), 0);
                memset(tmp, 0, sizeof(tmp));
          ...
          if (now_state == VISITOR) {
            ...
            if (found_comment == std::string::npos) {
              ...
              if (strlen(return_post) == 0) {
                ...
                rc = sqlite3_exec(post_db, tmp, find_post, (void *)data,
                                  &zErrMsg);
                if (rc != SQLITE_OK) {
                  fprintf(stderr, "SQL error: %s\n", zErrMsg);
                  sqlite3_free(zErrMsg);
                ...
                char ttmp[1024] = {0};
                sprintf(ttmp, "%s#%s#Comment successfully.\n%% ", now_user,
                        return_postowner);
                memset(return_postowner, 0, sizeof(return_postowner));
                // std::string response = "Comment successfully.\n% ";
                send(index, ttmp, sizeof(ttmp), 0);
                memset(return_post, 0, sizeof(return_post));
              }
            }
          }
        ...
        // -------------------------mail-to-------------------------
        else if (strncmp(cmd, "mail-to", 7) == 0) {
          find_user_fd(index);
          if (now_state == VISITOR) {
            std::string response = "Please login first.\n% ";
            send(index, response.c_str(), response.size(), 0);
          } else {
            char tmp[1024] = {0}, buf[1024] = {0}, subject[128] = {0},
                 content[1024] = {0}, *ret, *subject_pos, *content_pos;
            int content_len = 0;
            std::string checkargc(cmd, cmd + strlen(cmd));
            std::size_t founduser, foundsubject, foundcontent;
            founduser = checkargc.find("mail-to ");
            foundsubject = checkargc.find("--subject ");
            foundcontent = checkargc.find("--content ");
            if ((founduser == std::string::npos) ||
                (foundsubject == std::string::npos) ||
                (foundcontent == std::string::npos)) {
              std::string response = "Usage: mail-to <username> --subject "
                                     "<subject> --content <content>\n% ";
              send(index, response.c_str(), response.size(), 0);
            } else {
              ret = strstr(cmd, "mail-to ");
              subject_pos = strstr(cmd, "--subject ");
              content_pos = strstr(cmd, "--content ");
              bool send_content = false;

              memcpy(input_cmd_board, ret + 8 * sizeof(char),
                     subject_pos - (ret + 9 * sizeof(char)));
              memcpy(subject, subject_pos + 10 * sizeof(char),
                     content_pos - (subject_pos + 11 * sizeof(char)));
              strcpy(buf, content_pos + 10 * sizeof(char));
              ...
              memset(buf, 0, sizeof(buf));
              sprintf(buf,
                      "select Username from CLIENT where Username = \'%s\';",
                      input_cmd_board);
              // printf("%s\n", buf);

              // Execute SQL statement
              rc = sqlite3_exec(client_db, buf, find_user, (void *)data,
                                &zErrMsg);
              if (rc != SQLITE_OK) {
                fprintf(stderr, "SQL error: %s\n", zErrMsg);
                sqlite3_free(zErrMsg);
                ...
                fprintf(stdout, "find user successfully\n");
              }

              if (if_find_user == false) {
                sprintf(tmp, "%s does not exist.\n", input_cmd_board);
              } else {
                time_t t = time(0);
                char data_type1[64], data_type2[64];
                //   strftime(data_type1, sizeof(data_type1), "%Y/%m/%d",
                ...
                // puts(data_type2);
                memset(buf, 0, sizeof(buf));
                sprintf(buf, "%s VALUES (\'%s\', \'%s\', \'%s\', \'%s\');",
                        insert_to_mail, subject, now_user, data_type2,
                        input_cmd_board);
                printf("%s\n", buf);

                // std::string remailid = "select MailID from MAIL ";

                // Execute insert post
                rc = sqlite3_exec(mail_db, buf, callback, (void *)data,
                                  &zErrMsg);
                if (rc != SQLITE_OK) {
                  fprintf(stderr, "SQL error: %s\n", zErrMsg);
                  sprintf(tmp, "SQL error: %s\n", zErrMsg);
                  ...
                  memset(buf, 0, sizeof(buf));
                  sprintf(buf, "select MailID From MAIL where Subject = \'%s\'",
                          subject);
                  rc = sqlite3_exec(mail_db, buf, find_mailid, (void *)data,
                                    &zErrMsg);
                  if (rc != SQLITE_OK) {
                    fprintf(stderr, "SQL error: %s\n", zErrMsg);
                    sprintf(tmp, "SQL error: %s\n", zErrMsg);
                  ...
                  strcat(tmp, return_mail_id);
                  strcat(tmp, "#");
                  fprintf(stdout, "insert mail successfully\n");
                  strcat(tmp, "Sent successfully.\n");
                }
              }
              if_find_user = false;
              strcat(tmp, "% ");
              send(index, tmp, sizeof(tmp), 0);
              memset(input_cmd_board, 0, sizeof(input_cmd_board));
              memset(return_mail_id, 0, sizeof(return_mail_id));
              strcat(buf, "\n");
            }
          }
        }
        // --------------------------list-mail--------------------------
        else if (strncmp(cmd, "list-mail", 9) == 0) {
          find_user_fd(index);
          if (now_state == VISITOR) {
            std::string response = "Please login first.\n% ";
            send(index, response.c_str(), response.size(), 0);
          } else {
            char list_mail_msg[2048] = {0};
            // printf("111111\n");
            char select_the_mail[1024] = {0};
            sprintf(select_the_mail,
                    "select Subject, FromWho, Date From MAIL where ToWho = "
                    "\'%s\'",
                    now_user);

            // printf("-->%s\n", select_the_mail);
            rc = sqlite3_exec(mail_db, select_the_mail, list_mail, (void *)data,
                              &zErrMsg);
            if (rc != SQLITE_OK) {
              fprintf(stderr, "SQL error: %s\n", zErrMsg);
              sqlite3_free(zErrMsg);
            ...
            if (aaa.empty()) {
              // printf("empty---------\n");
              strcat(list_mail_msg, "empty");
            } else {
              strcat(list_mail_msg, tmp_list_mail);
            }
            // strcat(list_mail_msg, "% ");
            // printf("222222\n");
            // printf("-->%s\n", tmp_list_mail);
            send(index, list_mail_msg, sizeof(list_mail_msg), 0);
            memset(tmp_list_mail, 0, sizeof(tmp_list_mail));
            // printf("333333\n");
            list_i = 0;
          }
        }
        // ------------------------------retr-mail------------------------------
        else if (strncmp(cmd, "retr-mail", 9) == 0) {
          find_user_fd(index);
          std::string str_cmd(cmd, cmd + strlen(cmd));
          if (now_state == VISITOR) {
            std::string response = "Please login first.\n% ";
            send(index, response.c_str(), response.size(), 0);
          } else if (str_cmd.length() == 9) {
            std::string server_remsg = "Usage: retr-mail <mail#>\n% ";
            send(index, server_remsg.c_str(), server_remsg.length(), 0);
          } else {
            char list_mail_msg[2048] = {0};
            char select_the_mail[1024] = {0};
            char *mailid = nullptr;
            mailid = cmd + 10 * sizeof(char);
            ind = atoi(mailid);
            // printf("list_index is (%d)\n", ind);
            sprintf(select_the_mail,
                    "select ToWho, Subject, FromWho, Date, MailID From MAIL "
                    "where ToWho = "
                    "\'%s\'",
                    now_user);
            // printf("select_the_mail is (%s)\n", select_the_mail);

            rc = sqlite3_exec(mail_db, select_the_mail, find_mail, (void *)data,
                              &zErrMsg);
            if (rc != SQLITE_OK) {
              fprintf(stderr, "SQL error: %s\n", zErrMsg);
              sqlite3_free(zErrMsg);
            ...
            // printf("re_mail_id is -[%s]-\n", re_mail_id);
            if (list_index < ind) {
              strcat(list_mail_msg, "No such mail.\n% ");
              send(index, list_mail_msg, sizeof(list_mail_msg), 0);
              memset(re_mail_id, 0, sizeof(re_mail_id));
            } else {
              strcat(list_mail_msg, re_mail_id);
              send(index, list_mail_msg, sizeof(list_mail_msg), 0);
              memset(re_mail_id, 0, sizeof(re_mail_id));
            }
            list_index = 0;
        ...
        // ------------------------------delete-mail------------------------------
        else if (strncmp(cmd, "delete-mail", 11) == 0) {
          std::string str_cmd(cmd, cmd + strlen(cmd));
          find_user_fd(index);
          if (now_state == VISITOR) {
            std::string response = "Please login first.\n% ";
            send(index, response.c_str(), response.size(), 0);
          } else if (str_cmd.length() == 11) {
            std::string server_remsg = "Usage: delete-mail <mail#>\n% ";
            send(index, server_remsg.c_str(), server_remsg.length(), 0);
          } else {
            char list_mail_msg[2048] = {0};
            char select_the_mail[1024] = {0};
            char *mailid = nullptr;
            mailid = cmd + 12 * sizeof(char);
            ind = atoi(mailid);
            // printf("list_index is (%d)\n", ind);
            sprintf(select_the_mail,
                    "select ToWho, MailID From MAIL where ToWho = "
                    "\'%s\'",
                    now_user);
            // printf("select_the_mail is (%s)\n", select_the_mail);
            ...
            if (list_index < ind) {
              ...
              aaa += re_mail_id;
              foundid = aaa.find("#n#");
              std::string bbb = aaa.substr(foundid + 3);
              // printf("found at %lu\n", foundid);
              sprintf(tmp, "DELETE from MAIL where MailID = %s;", bbb.c_str());
              // printf("delete sql cmd is [%s]\n", tmp);
              rc = sqlite3_exec(mail_db, tmp, callback, (void *)data, &zErrMsg);
              if (rc != SQLITE_OK) {
                fprintf(stderr, "SQL error: %s\n", zErrMsg);
                sqlite3_free(zErrMsg);
              } else {
                fprintf(stdout, "Delete mail successfully\n");
              }
              // printf("mailid is (%s)\n", re_mail_id);
              strcat(list_mail_msg, re_mail_id);
              send(index, list_mail_msg, sizeof(list_mail_msg), 0);
